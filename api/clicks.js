...
export default async function handler(req, res) {
  ...
  const { id, days = '30' } = req.query; // id = link_id (de tu tabla link)
  ...
  // Traemos timestamps y agregamos en memoria (simple y suficiente)
  ...
  if (error) return res.status(500).json({ error: 'Error consultando clicks' });
